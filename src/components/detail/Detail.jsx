  ...
  return (
    <div className='flex-1'> {/*detail */}
      ...
      <div className='py-7 px-5 flex flex-col items-center gap-3 border-b-[1px] border-gray-600'>{/*User */}
        <img className='h-24 w-24 rounded-full object-cover' src={user?.avatar || "./avatar.png"} alt="" />
        <h2>{user?.username}</h2>
      ...
      <div className='flex flex-col p-5 gap-6'>{/*Info */}
        ...
        <div>{/*Option */}
          ...
          <div className='flex flex-col gap-5 mt-5'>{/*Photos */}
            <div className='flex items-center justify-between'>{/*PhotoItem */}
              ...
              <FileDownloadOutlinedIcon  className='p-1 bg-blue-950 opacity-45 rounded-full  cursor-pointer' style={{height:"2rem", width:"2rem"}} ></FileDownloadOutlinedIcon>
            </div>
        ...
        <button className='cursor-pointer py-2 px-5 bg-[rgba(230,74,105,0.533)] hover:bg-[rgba(220,20,60,0.733)] text-white border-none rounded-lg'
        onClick={handleBlock}>  {isCurrentUserBlocked
          ? "You are Blocked!"
          : isReceiverBlocked
          ? "User blocked"
          : "Block User"}</button>
        <button className='cursor-pointer py-2 px-5 bg-[rgba(23,37,84,0.5)] hover:bg-[rgba(29,78,216,0.7)] text-white border-none rounded-lg' onClick={() => auth.signOut()}>Log Out</button>
